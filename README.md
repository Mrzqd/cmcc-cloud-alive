...
按中文提示操作：选择主账户/子账户 → 登录 → 选云桌面 → **自己选择协议**（深信服 SCG / 中兴 ZTE）→ 开始保活。
...
**默认即为真实 live 保活**，无需额外环境变量。账号与云桌面在网页里添加；登录前可选择“主账户”或“子账户”，每张卡片可单独选协议、模式、间隔。
...
```bash
# 主账户（默认）
python3 -m cmcc_cloud_alive login <账号> --account-type main

# 子账户
python3 -m cmcc_cloud_alive login <子账户名> --account-type sub
```

账户类型会保存到对应档案中，token 失效后的自动重登会继续使用同一种登录类型。
